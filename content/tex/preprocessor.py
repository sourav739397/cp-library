...
def processraw(caption, instream, outstream, listingslang='raw'):
    try:
        ...
        if "Template" in caption:
            hash_script = 'hash'
            p = subprocess.Popen(['sh', f'content/contest/{hash_script}.sh'], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
...
# processraw and other functions: similar updates (decode, encoding)
